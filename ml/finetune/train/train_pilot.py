...
trainer.train()   # eval is off (OOMs on T4 at 4608), so watch TRAIN loss: it should fall then flatten;
                  # if it dives toward ~0.015 that's memorising — stop early. cell [9] smoke test is the real check.
...
model.save_pretrained_gguf("hermes-npc", tokenizer, quantization_method="q4_k_m", maximum_memory_usage=0.25)  # 0.5 OOM'd the free-box RAM on the fp16 merge -> shard harder
